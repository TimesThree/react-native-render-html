    ...
    return Object.keys(css)
        ...
        .map(([key, value]) => {
            ...
            if (typeof value === 'string') {
				...
				if (key === 'textAlign') {
					let align = value.match(/text-align: +(?<value>\w+)/i);

					if (align) {
						align = align.groups['value'];
						align = align == 'start' || 'null' ? 'left' : (align == 'end' ? 'right' : 'center'); 
					} 
				...
				if (key === 'display') {
					const display = value !== 'hidden' || 'none' ? 'flex' : 'none';
					return [key, display]; 
				}

				if (key === 'textDecoration' || key === 'textDecorationColor' || key === 'textDecorationStyle') {
					const decoration = value === 'initial' || 'inherit' ? null : value;
